<template>
    ...
    <div :class="getImageClass()" class="inline-block bg-stone-100/40 rounded-xs p-[2px] pl-[4px] pb-[4px] mx-2 bg-opacity-25">
        ...
        <img title="Abandoned Aqueduct" src="/img/risk_of_rain_2/abandoned_aqueduct.png" :class="{ 'opacity-10': !getNumberItemsFromName('Abandoned Aqueduct') && !stageUnlocked(1), 'opacity-35': xOrStageLevel('Abandoned Aqueduct', 1) }" />
        <img title="Wetland Aspect" src="/img/risk_of_rain_2/wetland_aspect.png" :class="{ 'opacity-10': !getNumberItemsFromName('Wetland Aspect') && !stageUnlocked(1), 'opacity-35': xOrStageLevel('Wetland Aspect', 1) }" />
        <img v-if="hasDLC1()" title="Aphelian Sanctuary" src="/img/risk_of_rain_2/aphelian_sanctuary.png" :class="{ 'opacity-15': !getNumberItemsFromName('Aphelian Sanctuary') && !stageUnlocked(1), 'opacity-35': xOrStageLevel('Aphelian Sanctuary', 1)  }" />
        <span class="mr-2"></span>
        <img title="Rallypoint Delta" src="/img/risk_of_rain_2/rallypoint_delta.png" :class="{ 'opacity-10': !getNumberItemsFromName('Rallypoint Delta') && !stageUnlocked(2), 'opacity-35': xOrStageLevel('Rallypoint Delta', 2) }" />
        <img title="Scorched Acres" src="/img/risk_of_rain_2/scorched_acres.png" :class="{ 'opacity-10': !getNumberItemsFromName('Scorched Acres') && !stageUnlocked(2), 'opacity-35': xOrStageLevel('Scorched Acres', 2)  }" />
        <img v-if="hasDLC1()" title="Sulfur Pools" src="/img/risk_of_rain_2/sulfur_pools.png" :class="{ 'opacity-10': !getNumberItemsFromName('Sulfur Pools') && !stageUnlocked(2), 'opacity-35': xOrStageLevel('Sulfur Pools', 2) }" />
        <span class="mr-2"></span>
        <img title="Abyssal Depths" src="/img/risk_of_rain_2/abyssal_depths.png" :class="{ 'opacity-10': !getNumberItemsFromName('Abyssal Depths') && !stageUnlocked(3),  'opacity-35': xOrStageLevel('Abyssal Depths', 3)  }" />
        <img title="Siren's Call" src="/img/risk_of_rain_2/siren_call.png" :class="{ 'opacity-10': !getNumberItemsFromName('Siren\'s Call') && !stageUnlocked(3), 'opacity-35': xOrStageLevel('Siren\'s Call', 3)  }" />
        <img title="Sundered Grove" src="/img/risk_of_rain_2/sundered_grove.png" :class="{ 'opacity-10': !getNumberItemsFromName('Sundered Grove') && !stageUnlocked(3), 'opacity-35': xOrStageLevel('Sundered Grove', 3) }" />
        <span class="mr-2"></span>
        <img title="Sky Meadow" src="/img/risk_of_rain_2/sky_meadow.png" :class="{ 'opacity-10': !getNumberItemsFromName('Sky Meadow') && !stageUnlocked(4),  'opacity-35': xOrStageLevel('Sky Meadow', 4) }" />
        <span class="mr-2"></span>
        <img title="Commencement" src="/img/risk_of_rain_2/commencement.png" :class="{ 'opacity-10': !getNumberItemsFromName('Commencement') && !stageUnlocked(4), 'opacity-35': xOrStageLevel('Commencement', 4)  }" />
    ...
    <div :class="getImageClass()" class="inline-block bg-stone-100/40 rounded-xs p-[2px] pl-[4px] pb-[4px] mx-2 bg-opacity-25">
        ...
        <img v-if="!limboRoute()" title="Hidden Realm: A Moment, Fractured" src="/img/risk_of_rain_2/a_moment_fractured.png" :class="{ 'opacity-35': !getNumberItemsFromName('Hidden Realm: A Moment, Fractured')  }" />
        <img v-if="!limboRoute()" title="Hidden Realm: A Moment, Whole" src="/img/risk_of_rain_2/a_moment_whole.png" :class="{ 'opacity-35': !getNumberItemsFromName('Hidden Realm: A Moment, Whole')   }" />
        <img title="Gilded Coast" src="/img/risk_of_rain_2/glided_coast.png" :class="{ 'opacity-35': !getNumberItemsFromName('Gilded Coast')  }" />
        <img title="Bulwark's Ambry" src="/img/risk_of_rain_2/bulwark_ambry.png" :class="{ 'opacity-35': !getNumberItemsFromName('Bulwark\'s Ambry')  }" />
        <img v-if="!voidlingRoute()" title="Hidden Realm: Bazaar Between Time" src="/img/risk_of_rain_2/bazaar_between_time.png" :class="{ 'opacity-35': !getNumberItemsFromName('Hidden Realm: Bazaar Between Time')   }" />
        <img v-if="!voidlingRoute()" title="Hidden Realm: Void Fields" src="/img/risk_of_rain_2/void_fields.png" :class="{ 'opacity-35': !getNumberItemsFromName('Hidden Realm: Void Fields')   }" />
        <img v-if="!voidlingRoute()" title="Hidden Realm: Void Locus" src="/img/risk_of_rain_2/void_locus.png" :class="{ 'opacity-35': !getNumberItemsFromName('Hidden Realm: Void Locus')   }" />
        <img v-if="!voidlingRoute()" title="Hidden Realm: The Planetarium" src="/img/risk_of_rain_2/the_planetarium.png" :class="{ 'opacity-35': !getNumberItemsFromName('Hidden Realm: The Planetarium')   }" />
    ...
    return {
        ...
        methods: {
            ...
            xOrStageLevel: function (stage, level) {
                if (!this.stageUnlocked(level) && this.getNumberItemsFromName(stage) ||
                    (this.stageUnlocked(level) && !this.getNumberItemsFromName(stage))) {
                    return true;
                }
                return false;
            },
